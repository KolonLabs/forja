...
| DT-004 | El modelo de beats globales y escenas derivadas se ha aplicado a `shared/relato`, pero todavía no dispone de una prueba de regresión con un relato que incluya inserciones `[D]`, reintentos y una edición derivada. | Una modificación futura puede reintroducir encabezados, contadores o agrupaciones incompatibles sin detectarse antes de crear workspaces. | Crear un fixture aislado y comprobar automáticamente IDs únicos, correspondencia guion/draft, transición de estados y regeneración del contexto desde una escena corregida. |
