...
`/recompilar-libro` no cambia contenido. Una corrección textual requiere `/nueva-edicion` (solo relatos por ahora). `/rehidratar-relato` vuelve a crear un workspace desde una semilla Forja; `/importar-proyecto` analiza notas o guiones libres, conserva sus evidencias fuera del workspace y permite confirmar la escala recomendada antes de crear el brief.
