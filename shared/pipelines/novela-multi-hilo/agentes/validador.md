---
description: Valida un beat o el draft completo en novelas multi-hilo. Soporta modo beat y modo global con Qdrant y validación cross-hilo.
mode: subagent
...
- Dimensión **coherencia** → `skill({ name: "validacion-coherencia" })` + `skill({ name: "validacion-cross-hilo" })`
...
8. **Contexto cross-hilo**: tabla de trenzado, `guion-hilo.md` de otros hilos que convergen en este capítulo, puntos de conexión declarados
...
10. **Contexto cross-hilo** (modo global) — tabla de trenzado del `guion-novela.md`, `guion-hilo.md` de los otros hilos que convergen en este capítulo, puntos de conexión declarados. El director te pasa esto para que puedas evaluar coherencia cross-hilo según el skill `validacion-cross-hilo`
