---
description: Valida un beat o el draft completo combinando múltiples dimensiones (crudeza, tono, geometría, coherencia, sensorial) según el scope solicitado.
mode: subagent
...
1. Draft completo (`relato-draft.md`)
2. `guion.md` completo (para verificar la intención de cada escena)
3. Fichas de personajes relevantes (desde `fichas/`) — para verificar consistencia
4. **Estilo activo**: nombre (de `config.json`)
5. Scope: `global` + dimensiones a evaluar (coherencia + tono)
...
3. **Fichas de personajes y lugar relevantes** del beat (desde `fichas/`, pasadas inline por el director)
...
6. **Beats anteriores**: los beats ya escritos de la escena actual (todos) + últimos 3 de la escena anterior si aplica — para evaluar continuidad en dimensión coherencia
...
```json
{
  "validador": "validador",
  "modo": "beat",
  "beat_stable_id": "a1b2c3d4", "beat_seq": 0005,
...
```json
{
  ...
  "problemas_globales": [
    {
      "beat_stable_id": "a1b2c3d4", "beat_seq": 0007,
      "dimensiones_afectadas": ["coherencia"],
      "score": 5.5,
      "problema": "El personaje X afirma no conocer a Y, contradiciendo lo establecido en B_0003",
      "sugerencia": "Ajustar el diálogo de B_0007 para ser consistente con el encuentro de B_0003"
    },
    {
      "beat_stable_id": "a1b2c3d4", "beat_seq": 0014,
  ...
  "feedback_resumen": "Contradicción de personaje en B_0007. Ruptura tonal en B_0014.",
  "correcciones_sugeridas": ["Corregir B_0007 por coherencia", "Ajustar tono de B_0014"]
}
```
