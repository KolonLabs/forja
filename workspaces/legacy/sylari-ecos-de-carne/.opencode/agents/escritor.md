---
description: Genera texto narrativo explícito para relatos y novelas. Escribe un beat cuando el director lo invoca.
mode: subagent
...
Esta nota **no aparece en `capitulo.md`** (es solo para el director/integrador/cronista). Se elimina en `/publicar-cap`. Pero ayuda a la trazabilidad durante el pipeline.
...
- **Estilo activo**: nombre (de `estado.json`) — base para cargar el skill `estilos-narrativos` con las reglas del estilo correspondiente
...
## Sincronización Qdrant — obligatoria en cualquier modo
...
- **Perfiles FIJO filtrados** de los personajes del beat
